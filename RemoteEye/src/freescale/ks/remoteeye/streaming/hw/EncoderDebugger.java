...
package freescale.ks.remoteeye.streaming.hw;

import java.io.PrintWriter;
import java.io.StringWriter;
import java.nio.ByteBuffer;

import freescale.ks.remoteeye.streaming.hw.CodecManager.Codec;
...
public class EncoderDebugger {
	...
	public synchronized static EncoderDebugger debug(SharedPreferences prefs, int width, int height) {
		...
		Log.e(TAG, "......new EncoderDebugger(prefs, width, height);......");
	

		int i=debugger.debug();
		Log.e(TAG, "####################"+i);
		if(i==-1)
		{
			debugger=null;
		...
		for (int i=0;i<encoders.length;i++) {
			for (int j=0;j<encoders[i].formats.length;j++) {
				...
				try {
					...
					// Starts the encoder
					Log.e(TAG, "........configureEncoder();........");
					configureEncoder();
					Log.e(TAG, "........configureEncoder(); OK........");
					
					long kk=searchSPSandPPS();
					if(kk==-1)
					{
						Log.e(TAG, "........can not find sps &pps........");
						return -1;
						//throw  new IllegalStateException("can not find sps pps, use another mothed");
						
					}
					Log.e(TAG, "........searchSPSandPPS(); OK........");
