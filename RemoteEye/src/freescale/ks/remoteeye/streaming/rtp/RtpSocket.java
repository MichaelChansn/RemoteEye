...
package freescale.ks.remoteeye.streaming.rtp;
...
import freescale.ks.remoteeye.streaming.rtcp.SenderReport;
...
public class RtpSocket implements Runnable {
	...
	private void sendTCP() throws IOException
		{
			...
			{
				int i = mPackets[mBufferOut].getLength();
				mTcpHeader[2] = (byte)(i >> 8);
				mTcpHeader[3] = (byte)(i & 0xff);
				mOutputStream.write(mTcpHeader);
				mOutputStream.write(mBuffers[mBufferOut], 0, i);
			}
	...
	public void setOutputStream(OutputStream outputstream, byte byte0)
	{
		if (outputstream != null)
		{
			...
			mTcpHeader[1] = byte0;
			mReport.setOutputStream(outputstream, (byte)(byte0 + 1));
		}
	}
	...
	public void start()
	{
		if (mThread == null)
		{
			mThread = new Thread(this, (new StringBuilder("RtpSocket - ")).append(Thread.currentThread().getName()).toString());
			mThread.start();
		}
	}
