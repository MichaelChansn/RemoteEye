...
package freescale.ks.remoteeye.streaming.rtsp;
...
import freescale.ks.remoteeye.streaming.Stream;
...
public class RtspClient {
		...
		mHandler.post(new Runnable () {
			...
			public void run() {
				...
				try {
					...
					Log.e(TAG,"...tryConnection() failed.....");
				...
				try {
					...
					Log.e(TAG,"...mParameters.session.syncStart() Start.....");
					mParameters.session.syncStart();
					mState = STATE_STARTED;
					if(mParameters.transport == 0)//udp
					{
	...
	private void tryConnection() throws IOException {
		...
		Log.e(TAG, ".......tryConnection()........");
		mCSeq = 0;
		mSocket = new Socket(mParameters.host, mParameters.port);
		mBufferedReader = new BufferedReader(new InputStreamReader(mSocket.getInputStream()));
		mOutputStream = mSocket.getOutputStream();
		Log.e(TAG, ".......sendRequestAnnounce() START........");
		sendRequestAnnounce();
		Log.e(TAG, ".......sendRequestAnnounce() OK........");
		Log.e(TAG, ".......sendRequestSetup() START........");
		sendRequestSetup();
		Log.e(TAG, ".......sendRequestSetup() OK........");
		//sendRequestRecord();
		sendRequestPlay();
		Log.e(TAG, ".......sendRequestPlay() OK........");
	}
	
	private void sendRequestPlay()
			throws IllegalStateException, SocketException, IOException
		{
			String s = (new StringBuilder("PLAY rtsp://")).append(mParameters.host).append(":").append(mParameters.port).append(mParameters.path).append(" RTSP/1.0\r\n").append("Range: npt=0.000-\r\n").append(addHeaders()).toString();
			Log.e(TAG,s.substring(0, s.indexOf("\r\n")));
			mOutputStream.write(s.getBytes("UTF-8"));
			mOutputStream.flush();
			Response.parseResponse(mBufferedReader);
			Log.e(TAG, ".......sendRequestPlay()........");
		}
	/**
	...
	private void sendRequestAnnounce() throws IllegalStateException, SocketException, IOException {
		...
		mOutputStream.flush();
		Log.e(TAG,"........sendRequestAnnounce() before Response.parseResponse(mBufferedReader) ...........");
		Response response = Response.parseResponse(mBufferedReader);
		Log.e(TAG,"........sendRequestAnnounce() after Response.parseResponse(mBufferedReader) ...........");

		if (response.headers.containsKey("server")) {
			Log.e(TAG,"RTSP server name:" + response.headers.get("server"));
		} else {
			Log.e(TAG,"RTSP server name unknown");
		}
	...
	private void sendRequestSetup() throws IllegalStateException, SocketException, IOException {
		...
		Log.e(TAG, "...........write setup begining...........");
		for (int i=0;i<2;i++) {
			Stream stream = null;
			
			
			 stream = mParameters.session.getTrack(i);
			
			
			if (stream != null) {
				String s1;
				String s = (new StringBuilder("/TCP;unicast;mode=receive;interleaved=")).append(i * 2).append("-").append(1 + i * 2).toString();
				if (mParameters.transport == 1)//tcp
				{
					s1 = s;
				}
				else//udp
				{
					s1 = (new StringBuilder(";unicast;mode=record;client_port=")).append(5000 + i* 2).append("-").append(1 + (5000 + i * 2)).toString();
				}
				String request = (new StringBuilder("SETUP rtsp://")).append(mParameters.host).append(":").append(mParameters.port).append(mParameters.path).append("/trackID=").append(i).append(" RTSP/1.0\r\n").append("Transport: RTP/AVP").append(s1).append("\r\n").append(addHeaders()).toString();
				
				Log.e(TAG, "...........write setup innnnnnnnnnnnnnng...........");
				Log.i(TAG,request.substring(0, request.indexOf("\r\n")));
				...
				{
					Log.e(TAG, "...........write setup failed...........");
				}
				Log.e(TAG,request);
				Log.e(TAG, "...........write setup oooooooooooooooooook...........");
				Response response = Response.parseResponse(mBufferedReader);
				
				try
				{
					Matcher matcher1 = Response.rexegSession.matcher((CharSequence)response.headers.get("session"));
					matcher1.find();
					mSessionID = matcher1.group(1);
				}
				...
				if (mParameters.transport == 0)
				{
					try
					{
						Matcher matcher = Response.rexegTransport.matcher((CharSequence)response.headers.get("transport"));
						matcher.find();
						stream.setDestinationPorts(Integer.parseInt(matcher.group(3)), Integer.parseInt(matcher.group(4)));
						Log.e(TAG,(new StringBuilder("Setting destination ports: ")).append(Integer.parseInt(matcher.group(3))).append(", ").append(Integer.parseInt(matcher.group(4))).toString());
					}
				...
				{
					stream.setOutputStream(mOutputStream, (byte)(i * 2));
				}
	...
	private String addHeaders() {
		...
		if (TextUtils.isEmpty(mSessionID))
			...
			s = (new StringBuilder("Session: ")).append(mSessionID).append("\r\n").toString();
		stringbuilder2 = stringbuilder1.append(s);
		if (mAuthorization != null)
			s1 = (new StringBuilder("Authorization: ")).append(mAuthorization).append("\r\n").toString();
		else
			s1 = "";
		return stringbuilder2.append(s1).append("\r\n").toString();
	}	
